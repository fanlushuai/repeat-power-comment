const Config = {
    ...
    setLSConfig2UI: function () {
        ...
        let textIds = ['keywords', 'commentCountLimit']
        let checkIds = []

        const regex = /id/ig
        for (let id of textIds) {
            let codeTemplate = 'let cf = configStorage.get("id") if (cf) { ui.id.setText(cf+"") }'
    ...
    setUI2LSConfig: function () {
        ...
        let textIds = ['keywords']
        const regex = /id/ig

        for (let id of numberIds) {
            let codeTemplate = 'let id = ui.id.getText() if (id && id != "") { configStorage.put("id", parseInt(id)) ;Config.id = parseInt(id) }'
            let oneParm = codeTemplate.replace(regex, id)
            // log(oneParm)
            eval(oneParm)
        }
        ...
        for (let id of textIds) {
            let codeTemplate = 'let id = ui.id.getText() if (id) { configStorage.put("id", id+"") ;Config.id = id+"" }'
            let oneParm = codeTemplate.replace(regex, id)
            // log(oneParm)
            eval(oneParm)
        }
    ...
    loadConfig: function () {
        ...
        let ids = ['commentCountLimit', 'keywords']

        const regex = /id/ig

        for (let id of ids) {
            let codeTemplate = 'let id = configStorage.get("id"); this.id = id; log("id %s",id)'
            let oneParm = codeTemplate.replace(regex, id)
            // log(oneParm)
            eval(oneParm)
        }
